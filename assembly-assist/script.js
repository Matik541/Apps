...
const elements = {
    ...
    markDirectionalButton: document.getElementById('markDirectionalButton'),
    mirrorXBtn: document.getElementById('mirrorXBtn'),
    mirrorYBtn: document.getElementById('mirrorYBtn'),
...
function drawOverlay() {
    ...
    ctx.fillStyle = elements.fontColorInput.value;
    ctx.strokeStyle = 'blue';
    ctx.lineWidth = 2 * state.scale * state.canvasScale;
    ctx.font = effectiveScale * state.scale * 1.5 + 'px ' + elements.fontFamilyInput.value;
...
function markDirectionalComponents() {
    const directionalKeywords = ['LED', 'JEDNOKIER', 'DIRECT', 'UNIDIRECT', 'UNI-DIRECT'];
    componentsData.forEach(comp => {
        const isDirectional = directionalKeywords.some(keyword =>
            comp.designator.toUpperCase().includes(keyword.toUpperCase())
        );
        comp.show = isDirectional;
    });
    renderTable();
    drawOverlay();
}
...
elements.markDirectionalButton.addEventListener('click', markDirectionalComponents);
...
elements.fontFamilyInput.addEventListener('input', drawOverlay);
elements.fontColorInput.addEventListener('input', drawOverlay);
